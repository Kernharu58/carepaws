...
- ✅ **Accounts** — user list with inline role (super_admin only) and status editing, identity-verification display, soft-delete/restore, and search/filter, all consistent with the backend's `/api/auth/users*` permission model.
- ✅ **ManagePets** — a lighter browsable gallery (cards, quick add/edit) alongside the more comprehensive PetManagement workspace.
- ⬜ Every other §7.2 page (Analytics, Adopter Profiles & Risk, Interviews/HomeVisits as standalone list pages, Document Review, Fosters, Monitoring, Health, Events, Shifts, Volunteer, Feedback, Content, Payments, Donations, Chat, Notifications, Staff, User Verification, Shelter Management, Audit Logs, Emergency Reports, Settings) — not yet built. `App.tsx`'s router and the sidebar's `navConfig.ts` only reference pages that exist, same practice as the backend's incremental route mounting.

### A real access-control gap found and fixed while building this

Building `Accounts` prompted a closer look at `App.tsx`'s routing, which turned up a real gap: the entire admin console's layout route had **no role restriction at all** — any authenticated user, including a plain `user`-role adopter account, could reach every staff page by URL (the sidebar hid the links, but hiding a link isn't access control). Fixed by gating the whole `AppLayout` route to `staff`/`admin`/`super_admin`, with `/accounts` further restricted to `admin`/`super_admin` on top of that. Worth being direct about: this was a real hole in a "production-ready" claim, caught only because building a page that manages roles made me look hard at how roles were actually enforced elsewhere.
